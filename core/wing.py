...
class FAN(nn.Module):
    def __init__(self, num_modules=1, end_relu=False, num_landmarks=98, fname_pretrained=None):
        ...
        # self.out = nn.Conv2d(num_landmarks + 1, 7, kernel_size=1)
    ...
    def load_pretrained_weights(self, fname):
        ...
        # checkpoint = torch.load('./checkpoints/Model_wing.pth')['Decoder']
        model_weights = self.state_dict()
        # model_weights.update({k: v for k, v in checkpoint.items()
    ...
    def forward(self, x):
        ...
        # tmp_out = self.out(tmp_out)  # added to segmentation
        if self.end_relu:
    ...
    @torch.no_grad()
    def get_heatmap(self, x, b_preprocess=True, Privacy=False, delimiter=False):
        ...
        if b_preprocess and Privacy:
            heatmaps = F.interpolate(heatmaps, scale_factor=scale_factor, mode='bilinear', align_corners=True)
            heatmaps = [heatmaps[:, :49].sum(dim=1, keepdim=True).clamp_(0, 1), heatmaps[:, 49:].sum(dim=1, keepdim=True).clamp_(0, 1)]
            # heatmaps = heatmaps.argmax(dim=1).unsqueeze(1).float()/6., heatmaps[:, [0, 2, 4, 5], ...].argmax(dim=1).unsqueeze(1).float()/3.
        elif not Privacy and delimiter:
            heatmaps = F.interpolate(heatmaps, scale_factor=scale_factor, mode='bilinear', align_corners=True)
            heatmaps[:, :33] = 0.
    ...
    def get_heatmap_train(self, x, b_preprocess=True, Privacy=False, delimiter=False):
        """ outputs 0-1 normalized heatmap """
        x = F.interpolate(x, size=256, mode='bilinear')
        x_01 = x * 0.5 + 0.5
        outputs, _ = self(x_01)
    ...
    @torch.no_grad()
    def get_heatmap_org(self, x, b_preprocess=True):
        ''' outputs 0-1 normalized heatmap '''
        x = F.interpolate(x, size=256, mode='bilinear')
        x_01 = x * 0.5 + 0.5
        outputs, _ = self(x_01)
    ...
    # @torch.no_grad()
    def estimate_landmark(self, img):
        x_01 = img * 0.5 + 0.5
        outputs, _ = self(x_01)
        feats = outputs[-1][:, :-1, :, :]
        feats[:, :33] = 0.
        ...
        for i in range(feats.size(0)):
            aux.append([torch.nonzero(feats[i, x, ...] == feats[i, x, ...].max())[0, :] for x in range(98)])
        points = torch.stack([torch.stack(sublist, dim=0) for sublist in aux], dim=0).float().unsqueeze(-1) * 4.
        return points

    @torch.no_grad()
    def get_landmark(self, x):
        """ outputs landmarks of x.shape """
        heatmaps = self.get_heatmap_org(x, b_preprocess=False)
        landmarks = []
