...
# flickr 30k vocab size: 7003
# flickr 8k vocab size: 2633
#coco 9490
encoder = Encoder()
...
def evaluate(beam_size, path):
    ...
    for batch_idx, (image, caps, caplens, allcaps) in enumerate(
        ...
        if batch_idx < 1000000:
            ...
            # image_camera = compose(image)
            # Forward of Original Image to DOE
            image_camera, psf, zernike_coeffs_train, loss_psf = camera(image, None, "3")

            info_str = ""
            if aux < limite and (batch_idx + 1) % 50 == 0:
                psnr = PSNR(image, image_camera)
                ssim = SSIM(image, image_camera)
                info_str += "\t {:.0f} \t {:.3f} \t {:.3f} ".format(aux, psnr, ssim)
                plt.imshow(image[0].squeeze().permute(1, 2, 0).detach().cpu().data), plt.axis('off'), plt.savefig(
                    imgs_path + '/org_{:.0f}.png'.format(aux))
                plt.imshow((image_camera[0]/image_camera[0].max()).squeeze().permute(1, 2, 0).detach().cpu().data), plt.axis('off'), plt.savefig(
                    imgs_path + '/len_{:.0f}.png'.format(aux))

            # Encode
            encoder_out = encoder(image_camera)  # (1, enc_image_size, enc_image_size, encoder_dim)
...
path = '../results/GPU_final/'
#path = '../Checkpoint/'
dir_checkpoint = path + 'checkpoint_'+name+'_5_cap_per_img_5_min_word_freq.pth'
...
del checkpoint
compose = transforms.Compose([transforms.Resize((16, 16)), transforms.Resize((256, 256))])
...
f.write('\tIndex \tPSNR \tSSIM \t\t\t\t ORG \t\t\t\t\t EST \n\n')
