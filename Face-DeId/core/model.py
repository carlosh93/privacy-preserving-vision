...
class Generator(nn.Module):
    def __init__(self, img_size=256, style_dim=64, max_conv_dim=512, w_hpf=1):
        ...
        '''self.rgb_translate = nn.Sequential(nn.Conv2d(6, 18, 3, 1, 1),
                                           nn.Conv2d(18, 9, 3, 1, 1),
                                           nn.Conv2d(9, 3, 3, 1, 1)
                                           )'''
...
def build_model(args):
    ...
    if args.w_hpf > 0:
        ...
        fan.estimate_landmark = fan.module.estimate_landmark
        nets.fan = fan
        nets_ema.fan = fan

        fan_priv = nn.DataParallel(FAN().eval())
        # fan_priv = nn.DataParallel(FAN(fname_pretrained=args.wing_path))
        fan_priv.get_heatmap = fan_priv.module.get_heatmap
        nets.fan_priv = fan_priv
        nets_ema.fan_priv = fan_priv
