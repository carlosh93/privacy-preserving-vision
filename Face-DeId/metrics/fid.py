...
class FID(nn.Module):
    def __init__(self):
        ...
        self.inception = InceptionV3().eval().to(device)
    ...
    def forward(self, batches):
        ...
        for batch in batches:
            actvs = []
            actv = self.inception(batch)
            actvs.append(actv)
            actvs = torch.cat(actvs, dim=0).cpu().detach().numpy()
            mu.append(np.mean(actvs, axis=0))
            cov.append(np.cov(actvs, rowvar=False))
        return frechet_distance(mu[0], cov[0], mu[1], cov[1])
...
@torch.no_grad()
def calculate_fid_given_batch(batches, img_size=256, batch_size=50):
    print('Calculating FID given a Batches')
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    inception = InceptionV3().eval().to(device)
